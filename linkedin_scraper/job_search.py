...
class JobSearch(Scraper):
    ...
    def scrape_logged_in(self, close_on_complete=True, scrape_recommended_jobs=True):
        ...
        if scrape_recommended_jobs:
            ...
            job_area = self.wait_for_element_to_load(By.CLASS_NAME, "scaffold-finite-scroll__content")
            areas = self.wait_for_all_elements_to_load(By.CLASS_NAME, "artdeco-card", base=job_area)
    ...
    def search(self, search_term: str) -> List[Job]:
        ...
        url = urljoin(self.base_url, "search") + f"?keywords={urllib.parse.quote(search_term)}&refresh=true"
        self.driver.get(url)
        ...
        job_results = []
        for job_card in self.wait_for_all_elements_to_load(By.CLASS_NAME, "job-card-list", base=job_listing):
